...
```bash
# From the dockervm host, in the kuato/postgres directory
...
docker run --rm -d --name kuato-restore-test \
...
# Restore
gunzip -c backups/last/claude_sessions-latest.sql.gz \
  | docker exec -i kuato-restore-test \
      psql -U claude -d claude_sessions

# Poke around
docker exec -it kuato-restore-test psql -U claude -d claude_sessions -c "SELECT COUNT(*) FROM sessions;"

# Tear down
docker rm -f kuato-restore-test
```
